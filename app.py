...
# HARUS MENJADI BARIS PERTAMA SETELAH IMPORT STREAMLIT
st.set_page_config(
...
# BARU SETELAH INI BOLEH IMPORT LAINNYA
import pandas as pd
import numpy as np
import os
import sys
...
if sys.version_info >= (3, 12):
    st.warning(f"⚠️ Python {python_version} detected. Using compatibility mode.")
...
try:
    ...
    st.error(f"❌ ML libraries not available: {str(e)}")
    st.info("ℹ️ Running in simplified mode")
...
def display_header():
    ...
    if os.path.exists(image_path):
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                    padding: 2rem; text-align: center; border-radius: 15px; color: white; margin-bottom: 1rem;">
            <h1>🚕 SIGMA CABS</h1>
...
def calculate_surge_pricing(distance, rating, cab_type, traffic, demand, weather):
    ...
    try:
        ...
        cab_factor = cab_factors.get(cab_type, 0.0)
...
def main():
    ...
    with info_col1:
        ...
        <div class="info-box">
            ...
            <p><strong>ML Libraries:</strong> {'✅ Available' if ML_AVAILABLE else '❌ Not Available'}</p>
            <p><strong>Algorithm:</strong> {'Advanced ML' if ML_AVAILABLE else 'Rule-Based'}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with info_col2:
        ...
        <div class="info-box">
            <h4>📊 Features</h4>
            <p><strong>Surge Prediction:</strong> ✅ Active</p>
            <p><strong>Real-time Factors:</strong> ✅ Supported</p>
            <p><strong>Mobile Friendly:</strong> ✅ Responsive</p>
        </div>
        """, unsafe_allow_html=True)
